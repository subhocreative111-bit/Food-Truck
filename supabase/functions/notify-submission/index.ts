...
//
// Deno-runtime Supabase Edge Function. Wired up to two Database Webhooks:
//   - INSERT on public.claims        → "New claim from $name"
//   - INSERT on public.submissions   → "New contact: $subject"
//
// Sends via Resend (https://resend.com) using a single API key set as a
// Function Secret on the project. Falls back to a console log if RESEND_API_KEY
// isn't set, so deploys never break — just no email gets sent.
//
// Deploy via the Supabase MCP:
//   deploy_edge_function({ name: "notify-submission", entrypoint_path: "index.ts" })

// @ts-expect-error: Deno-only import path; resolved at runtime in Supabase Edge runtime
import { serve } from 'https://deno.land/std@0.224.0/http/server.ts';

declare const Deno: { env: { get(key: string): string | undefined } };

const RESEND_API_KEY = Deno.env.get('RESEND_API_KEY');
const ADMIN_EMAIL    = Deno.env.get('ADMIN_EMAIL') ?? 'hello@foodtrucksnearmeusa.com';
const FROM_EMAIL     = Deno.env.get('NOTIFY_FROM') ?? 'FoodTrucksNearMeUSA <noreply@foodtrucksnearmeusa.com>';
...
function renderClaim(r: Record<string, unknown>): { subject: string; html: string; text: string } {
  ...
  const dash = `https://supabase.com/dashboard/project/${Deno.env.get('SUPABASE_PROJECT_REF') ?? '_'}/editor`;
  const html = `
    <div style="font-family: system-ui, -apple-system, Segoe UI, sans-serif; line-height: 1.6;">
      ...
      <p style="margin-top: 24px;">
        <a href="${dash}" style="display: inline-block; background: #C8463A; color: #FAF4E8; padding: 10px 20px; border-radius: 999px; text-decoration: none; font-weight: 700;">
          Review in Supabase &rarr;
        </a>
      </p>
    </div>
  `;
...
function renderSubmission(r: Record<string, unknown>): { subject: string; html: string; text: string } {
...
serve(async (req: Request) => {
  ...
  if (!rendered) {
    return new Response(JSON.stringify({ ok: true, skipped: `unknown table ${payload.table}` }), {
      headers: { 'Content-Type': 'application/json' },
    });
  }
